...
import { AlertTriangle, Lightbulb, FastForward, X } from 'lucide-react';
      ...
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-950 p-6">
        <div className="mb-4 flex items-start justify-between">
          <div>
            ...
            <h2 className="mt-1 text-2xl font-bold text-white">{isHint ? 'Buy a hint' : 'Skip checkpoint'}</h2>
          </div>
        ...
        <div className="mb-4 rounded-2xl border border-amber-400/20 bg-amber-400/10 p-4 text-amber-100">
          <div className="flex items-start gap-3">
            {isHint ? <Lightbulb className="mt-0.5 h-5 w-5" /> : <FastForward className="mt-0.5 h-5 w-5" />}
