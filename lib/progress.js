...
export async function buildProgressPayload(team) {
  ...
  const finalUnlocked = allComplete && (team.score || 0) >= config.MIN_POINTS_THRESHOLD;
  ...
  return {
    ...
    lifelineCosts: { HINT: config.HINT_COST, SKIP: config.SKIP_COST },
    finalUnlocked,
    ...
    lockReason: !allComplete
      ...
      : (team.score || 0) < config.MIN_POINTS_THRESHOLD
